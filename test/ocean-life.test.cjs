...
function fixture({reduce = false} = {}) {
  ...
  const encounter = {clientWidth:1200, style:{setProperty(){}}, classList:{toggle(name,on){on ? classes.add(name) : classes.delete(name)}},
    querySelector(){return water}, getBoundingClientRect(){return {top:200-context.scrollY,height:1000}}};
  ...
  return {document,media,classes,pending,canvas,draws:()=>draws,
...
test('sea life runs only on visible sections and stops scheduling when paused', () => {
  ...
  p.step(100);const draws=p.draws();p.pause(true);assert.equal(p.pending.size,0);assert.ok(p.classes.has('sea-life-stopped'));
  assert.equal(p.draws(),draws);p.pause(false);assert.equal(p.pending.size,1);
...
test('background tabs stop the animation and reduced motion never starts it', () => {
  ...
  const quiet=fixture({reduce:true});quiet.visible(true);assert.equal(quiet.pending.size,0);
});
