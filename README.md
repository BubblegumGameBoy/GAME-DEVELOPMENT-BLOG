...
>
> 2026-07 RPG化（第1弾）: トップに「ワールドマップ」セクションを追加。各ページを
> RPGの拠点（ボスの城／パチパチ平原／としょかん／ぼうけんギルド 等）に見立てた
> クリッカブルマップ（CSS+インラインSVGのピクセルアートのみ、JS不要）。
> AdSense 審査中のため、ナビの点滅「裏サイト」リンクは撤去し、マップ上の
> 「？？？」の洞窟とフッターの地味リンクのみに降格（承認後に戻すか再判断）。

## ディレクトリ構成

```
index.html                     トップページ（ヒーロー / ワールドマップ / いちおし / 新着 / ゲーム一覧 / about / プライバシー / お問い合わせフォーム）
...
- RPG化の次弾候補: ①ゲームページの本文強化（遊び方・攻略・世界観＝thin content対策を兼ねる）
  ②localStorage によるプレイヤーLv/クエスト（B案） ③世界観でのナラティブ接続（C案）。
  ※コンテンツをゲート（演出の裏に隠す）しないこと。本文は常に素のHTMLで置く。
